...
// ── Shared UI helpers ──────────────────────────────────────────────────────
const PALETTE = ["bg-indigo-500","bg-purple-500","bg-pink-500","bg-blue-500",
...
function BAUSection() {
  ...
  const [data, setData]         = useState<BAUData | null>(null);
  const [loading, setLoading]   = useState(false);
  const [error, setError]       = useState<string | null>(null);
  const [page, setPage]         = useState(1);

  async function load(t: string, f?: string, toD?: string, n?: string) {
  ...
  // Reload when tab changes, reset filters
  useEffect(() => {
    setFrom(""); setTo(""); setNameQ("");
    load(bauTab);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bauTab]);

  const handleApply = () => load(bauTab, from, to, nameQ);
  const handleReset = () => { setFrom(""); setTo(""); setNameQ(""); load(bauTab); };
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {pagedRows.length === 0 ? (
                  <tr>
                    <td colSpan={data.headers.length} className="px-4 py-10 text-center text-gray-400">
                      No records found
                    </td>
                  </tr>
                ) : pagedRows.map((row, i) => (
    ...
    <div className="space-y-5">
      ...
      <div style={{ display: section === "kpi" ? "block" : "none" }}>
        <div className="space-y-8">
          ...
          {!loading && !error && data && (
            <>
              {/* ══ SECTION 1: CR Pax KPI DB & CR Team KPI DB ══ */}
